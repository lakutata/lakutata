...
const getAvailablePort = async (
    ...
    for (const host of hosts) {
        try {
            ...
            if (!['EADDRNOTAVAIL', 'EINVAL'].includes((<any>error).code)) {
                throw error
            }
        }
    }
...
export default async function pickFreePort(
    options?: {
        port?: number | number[];
    ...
    if (options) {
        ...
        if (options.exclude) {
            ...
            exclude = new Set<number>(excludeIterable)
        }
    }
    ...
    for (const port of portCheckSequence(ports)) {
        try {
            ...
            while (
                ...
                if (port !== 0) {
                    throw new Locked(availablePort)
                }
            ...
            if (
                !['EADDRINUSE', 'EACCES'].includes((<any>error).code) &&
...
export function portNumbers(from: number, to: number): Generator<number> {
    ...
    function* generator(from: number, to: number): Generator<number> {
